...
void UserDecayConfig() {
   ...
   for (Int_t i = 0; i < 6; i++) {   // delete all possible decay modes ratios to 0;
      bratio[i] = 0.;
      mode[i][0] = 0;
      mode[i][1] = 0;
      mode[i][2] = 0;
   ...
   gMC->SetDecayMode(3312,bratio,mode);  // this is the important methode for this macro implementet in the instance of the transport system used by the Virtual Monte Carlo system
   	   	   	   	   	   	   	   	   	   	 // it changes the Decay modes and ratios for the specified particle (pdg code), all other decays stay untouched (at least I think so)

   // lets do the same for Anti Xi *****************************

   for (Int_t i = 0; i < 6; i++) {   // delete all possible loaded/definded decay modes and set ratios to 0;
      bratio[i] = 0.;
      mode[i][0] = 0;
      mode[i][1] = 0;
      mode[i][2] = 0;
   ...
   gMC->SetDecayMode(3122,bratio,mode);  // this is the important methode for this macro implementet in the instance of the transport system used by the Virtual Monte Carlo system
   	   	   	   	   	   	   	   	   	   	 // it changes the Decay modes and ratios for the specified particle (pdg code), all other decays stay untouched (at least I think so)
